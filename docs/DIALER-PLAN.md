...
reason is not tidiness: the consoles used to share an ORIGIN, and therefore
a `sessionStorage`, with `dashboard.html` — 2.3MB and 37k lines. One XSS
anywhere in that file yielded a session that can place calls on a live
...
- **Cutover.** `staffportal.../dialer/` and `/dialer/admin.html` are redirect
  stubs now, and the dashboard's two Dialer buttons open the subdomains
  directly. Until this landed the split bought nothing at all: the old copy
  was still there to be opened, on the origin the whole exercise exists to get
  off. The stubs are generated by `tools/publish-dialer.py --stubs` so the
  old-path-to-new-host mapping has one home, they carry the query and hash
  across so a deep link survives, and they say in a sentence why the new
  address asks for a sign-in. Verified end to end:
  `staffportal.../dialer/admin.html?campaign=abc#tab-lists` lands on
  `admin.proptechnologyai.com/?campaign=abc#tab-lists`.

  `noopener` is still NOT set on the `window.open`, and the reason changed.
  The old one — the opener relationship is what clones `sessionStorage` into
  the dialer tab — is dead across origins. What replaces it is duller:
  `window.open(..., 'noopener')` returns `null` by specification, and the
  popup-blocker fallback on the next line reads `null` as "blocked" and
  answers by navigating THIS tab away, so an agent mid-call would lose the
  portal. The destination is our own page; the opener handle costs less
  than that.

  Rollback is `git revert` of that commit on `live`.

**Left**

Nothing, but one thing deliberately NOT done: the two origins are **not** in
Supabase Auth's redirect allowlist, because nothing consults it. Both consoles
call only `signInWithPassword` — no `redirectTo`, no OAuth, no magic link, no
`resetPasswordForEmail` — and the "forgotten it?" link sends people to the
staff portal, which is allowlisted already. Add them if a password reset is
ever put on the consoles themselves, not before.
